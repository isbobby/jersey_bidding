...
def validateChoiceAvailable(desiredNumber, user):
    # check if isTaken
    desiredJersey = JerseyNumber.query.filter((JerseyNumber.gender_id == user.gender_id) & (JerseyNumber.number == desiredNumber)).first()
    ...
    if UserPlayMixedSport(user):
        ...
        desiredJerseyOppositeGender = JerseyNumber.query.filter((JerseyNumber.gender_id == oppositeGenderID) & (JerseyNumber.number == desiredNumber)).first()
...
def allocateByYear(currentYear):
    ...
    UsersFromCurrentYear = User.query.filter(User.year == currentYear).order_by(User.points.desc()).all()
